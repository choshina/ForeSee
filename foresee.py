...
def conf_gen(home, rq, time_str, specID, algo, trials, c = 0.2, bp = 10):
	...
	if rq == 1:
		...
		else if specID.startswith('AFC'):
			model = 'fuel_control'
			basic_file = home + '/test/conf/AFC_basic'
		else if specID.startswith('FFR'):
	...
	if algo == 'foresee':
		os.system('python foresee_gen.py ' + conf_file)
	else if algo == 'breach':
		os.system('python breach_gen.py ' + conf_file)
		...
		with open(res_file, 'r') as res:
			...
			for line in lines[1:]:
				...
				if status == trials:
					...
					row = ';'.join(data[0:2]) + ';'+str(fal)+';'+str(time)+';'+str(-1) + ';'+str(numsim)
					f.write(row+'\n')
					fal = 0
					time = 0
					numsim = 0
