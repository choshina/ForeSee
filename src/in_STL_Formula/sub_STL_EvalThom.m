function [val__, time_values__] = sub_STL_EvalThom(Sys, phi, P, trajs, seq, t)
...
for ii=1:numTrajs % we loop on every traj in case we check more than one
    ...
    if exist('t','var')
        ...
        [val, time_values] = sub_GetValues(Sys, phi, Pii, traj, seq, interval);
        ...
        [val__ii, time_values__ii] = sub_GetValues(Sys, phi, Pii, traj, seq, interval);
...
%The normal GetValues, just for judging if w satisfies phi.
...
%Our GetValues, with seq as an argument
function [valarray, time_values] = sub_GetValues(Sys, phi, P, traj, seq, interval)
global BreachGlobOpt;
eval(BreachGlobOpt.GlobVarsDeclare);
...
switch(phi.type)
    ...
    case 'not'
        [valarray, time_values] = sub_GetValues(Sys, phi.phi, P, traj, seq, interval);
        valarray = - valarray;
        
    case 'or'
        val = [];
        timev = [];
        sub_forms = My_STL_Break(phi);
        for f = sub_forms
            if equals_to(seq.head(), f)
                [valarray_s, time_values_s] = sub_GetValues(Sys, f, P, traj, seq.suffix(), interval);
            else
                [valarray_other, time_values_other] = GetValues(Sys, f, P, traj, interval);
                if numel(val) == 0
                    val = valarray_other;
                    timev = time_values_other;
                else
                    [timev, val] = RobustOr(timev, val, time_values_other, valarray_other);
                end
            end
        end
        %TOCHECK: assume the idx is available in valarray_s
        %[unzip_valarray_s, unzip_time_values_s] = unziparr(valarray_s, time_values_s, traj.time);
        %[unzip_val, unzip_timev] = unziparr(val, timev, traj.time);
        ...
        val(sat_idx) = intmax;
        val(vio_idx) = intmin;
        [time_values, valarray] = RobustOr(time_values_s, valarray_s, timev, val);
        
        
        
        %[valarray, time_values] = ziparr(unzip_valarray_s, unzip_time_values_s, time_values_s);
    ...
    case 'and'
        ...
        for f = sub_forms
            if equals_to(seq.head(), f)
                ...
                if numel(val) == 0
                    ...
                    [timev, val] = RobustAnd(timev, val, time_values_other, valarray_other);
                end
            end
        end
        %
        ...
        val(sat_idx) = intmax;
        val(vio_idx) = intmin;
        [time_values, valarray] = RobustAnd(time_values_s, valarray_s, timev, val);
    ...
    case 'always'
        ...
        I___(1) = min(I___(1), I___(2));
        next_interval = I___+interval;
        [valarray, time_values] = sub_GetValues(Sys, phi.phi, P, traj, seq, next_interval);
        if(I___(end)~=inf)
            time_values = [time_values time_values(end)+I___(end)];
            valarray = [valarray valarray(end)];
        end
    ...
    case 'eventually'
        ...
        I___(1) = min(I___(1), I___(2));
        next_interval = I___+interval;
        [valarray1, time_values1] = sub_GetValues(Sys, phi.phi, P, traj, seq, next_interval);
        if(I___(end)~=inf)
            time_values1 = [time_values1 time_values1(end)+I___(end)];
            valarray1 = [valarray1 valarray1(end)];
        end
...
function sub_RobustOr()
    
end

function sub_RobustAnd()
...
%[unzip_valarray_s, unzip_time_values_s] = unzip(valarray_s, time_values_s, traj.time);
