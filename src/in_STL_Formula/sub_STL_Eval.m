function [val, tau] = sub_STL_Eval(Sys, phi, P, trajs, seq, varargin)
...
if ischar(phi)
    ...
    [val__, time_values__] = STL_EvalThom(Sys, phi_tmp__, P, trajs, seq, t);
...
switch nargin
    case 5
        [val, tau] = sub_STL_EvalThom(Sys, phi, P, trajs, seq);
    case 6
        if ~ischar(varargin{1})
            [val, tau] = sub_STL_EvalThom(Sys, phi, P, trajs, seq, varargin{1});
        elseif strcmpi(varargin{1},'classic')
            [val, tau] = sub_STL_EvalClassic(Sys, phi, P, trajs, seq);
        elseif strcmpi(varargin{1},'online')
            [val, tau] = sub_STL_EvalClassicOnline(Sys, phi, P, trajs, seq);
        else
            [val, tau] = sub_STL_EvalThom(Sys, phi, P, trajs, seq);
        end
end
